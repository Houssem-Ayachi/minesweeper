...
const uiContainer: Element = document.querySelector("#uiContainer")!;

const options: GridOptions = {rows: 14, cols: 18, numberBombs: 40, squareWidth: 40};
...
const gameAlert = (message: string) => {
    setTimeout(() => {
        alert(message);
        restart();
    }, 100);
...
canvas.addEventListener("mousedown", e => {
    ...
    if(e.buttons === 4){
        ...
        if(bombFound) gameAlert("You LOST!");
    }
});
